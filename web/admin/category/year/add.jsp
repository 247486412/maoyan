...
<form id="toForm" class="form-inline" method="post"
      action="${pageContext.request.contextPath}/AdminYearServlet?method=addYear">
    <div class="form-group ">
        <label for="exampleInputName2" class="h4">年份:</label>
        <input type="text" class="form-control inputText" id="exampleInputName2" name="name" placeholder="请输入年份">
        <%--button默认会是submit需要给type--%>
        <button type="button" class="btn btn-default addCategory">添加多个</button>
        <a href="${pageContext.request.contextPath}/AdminYearServlet?method=findAllYear"><button type="button" class="btn btn-default" id="goBack">返回</button></a>
